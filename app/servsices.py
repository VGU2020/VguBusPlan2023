...
def obj_to_geojson(network, vehicle_positions):
    ...
    for _, v in vehicle_positions.iterrows():
        ...
        stations = network.stop_times.loc[network.stop_times.trip_id == v.trip_id, "stop_id"].values.tolist()
        for st_id in stations:
            station = network.stops.loc[network.stops.stop_id == st_id]
            stop = Stop(
            ...
            stop_collection["features"].append(stop_feature)
        ...
        i = 0
        while i <= len(shape_coords):
            segment_coords = None
            if len(shape_coords) - i <= 10:
                segment_coords = shape_coords[i:(len(shape_coords)-1)]
                segment = Segment(
                    trip_id=v.trip_id,
                    congestion_lv=random.randint(0, 5),
                    seg_coords=segment_coords
                )
                segment_feature = segment.to_geojson()
                segment_feature["Popup"] = make_html(segment_feature["properties"])
                segment_collection["features"].append(segment_feature)
                break
            else:
                a = i + random.randint(10, 15)
                segment_coords = shape_coords[i:a]
                segment = Segment(
                    trip_id=v.trip_id,
                    congestion_lv=random.randint(0, 5),
                    seg_coords=segment_coords
        ...
        collection["features"].append(stop_collection)
        collection["features"].append(path_feature)
        collection["features"].append(segment_collection) 
